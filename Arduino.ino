...
QTRSensorsAnalog qtra((unsigned char[]) {A3,A2,A1,A0,A7,A6,A5,A4}, 
  NUMSENSORS, 4, 22);
...
void setup() {
  ...
  {
    qtra.calibrate();
    delay(20);// reads all sensors 10 times at 2.5 ms per six sensors (i.e. ~25 ms per call)
  }
...
void followLine(int velocidadeA, int velocidadeB, bool sa, bool sb){
  ...
  analogWrite(PWMA, velocidadeA);
  analogWrite(PWMB, velocidadeB);
